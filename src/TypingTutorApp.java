...
public class TypingTutorApp {
    ...
    private static FallingWord[] words, hungryWords;
    static WordMover[] wrdShft;
    static WordMover[] hWrdShft;
    ...
    public static void setupGUI(int frameX, int frameY, int yLimit) {
        ...
        JLabel scr = new JLabel("typingTutor.Score:" + score.getScore() + "    ");
...
//
    ...
    public static void createWordMoverThreads() {
        score.reset();  //initialize shared array of current words with the words for this game
        for (int i = 0; i < (noWords); i++) 
        {
            words[i] = new FallingWord(dict.getNewWord(), gameWindow.getValidXpos(), yLimit ); // creating falling word objects
        }
        
        words[noWords] = new HungryWordMover(dict.getNewWord(), 900); //Creating Hungry Word falling word objects
        words[noWords].setHungry(true);
        
        for (int i = 0; i < (noWords + 1); i++)
        {
                wrdShft[i] = new WordMover(words[i], dict, score, startLatch, done, pause); //creating Wordmover objects
        }
    ...
    public static void main(String[] args) {
        ...
        words = new FallingWord[noWords + 1]; // will contain both hungryWords and falling words
        //hungryWords = new typingTutor.FallingWord[noWords];//array for the  current chosen words from dict
        wrdShft = new WordMover[noWords + 1]; //array for the threads that animate the words
//        wrdShftH = new typingTutor.HungryWordMover[noWords]; //array for the threads that animate the words
